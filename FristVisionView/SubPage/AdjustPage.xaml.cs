...

    {
        ...
        private void CardMouseMove(object sender, MouseEventArgs e)
        {
            ...
            foreach (var card in _DragStartPoint)
            {
                ...
                var wireX = Math.Round(newX/ gridSize) * gridSize;
                var wireY = Math.Round(newY  / gridSize) * gridSize;
                ...
                foreach (var wire in vm.AllWires)
                {
                    if (wire.SourceCard == cardKey || wire.TargetCard == cardKey)
                    {
                        ...
                        {
                            wire.StartPoint = new Point(wire.StartPoint.X + wireX, wire.StartPoint.Y + wireY);

                        }
                        if (wire.TargetCard == cardKey)
                        {
                            wire.EndPoint = new Point (wire.EndPoint.X + wireX, wire.EndPoint.Y + wireY );
                        }
                        wire.UpdatePath();
                    }
